...
pub trait PairingOps<P: PacketPool> {
    ...
    /// Whether this connection used a local Resolvable Private Address (i.e. the
    /// peripheral advertised with an RPA). The peripheral should only distribute
    /// its IRK when this is true; distributing an IRK while advertising with a
    /// static address is meaningless to the peer and is rejected by some centrals
    /// (notably macOS).
